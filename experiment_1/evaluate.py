...
def get_commits(revs, changed_files, extensions):
    hashcodes = tuple(hash for hash in rc(
        'git --no-pager log --first-parent --pretty=tformat:"%H"')[1].split() if hash)
    ...
    with cd(REPOS):
        for r in experiment.repos:
            ...
            with cd(r.name):
                ...
                try:
                    rc(f'git checkout {r.starting_commit}')
                except Exception:
                    pass
                r.commits = get_commits(
                    experiment.revs, experiment.changed_files, experiment.extensions)


def preprocess_repos(experiment):
    if 'generated' not in experiment:
        experiment.generated = []
    if 'exclude' not in experiment:
        experiment.exclude = []
    for r in experiment.repos:
        r.git = r.url + '.git'
        r.name = r.url.rsplit('/', 1)[-1]
        if 'src_folder' not in r:
            r.src_folder = experiment.src_folder
        if 'test_folder' not in r:
            r.test_folder = experiment.test_folder
...
def process_repos(experiment):
    ...
    tools = tuple((tool, globals()[func])
                  for tool, func in experiment.tools.items())
    run_babelrts_tests = globals()[experiment.run_babelrts_tests]
    for r in experiment.repos:
        print(r.name)
        with cd(join(REPOS, r.name)):
            babelRTS = BabelRTS('.', r.src_folder, r.test_folder,
                                experiment.exclude, experiment.languages)
            for i, commit in enumerate(r.commits):
                ...
                try:
                    ...
                    for tool, func in tools:
                        commit.tools[tool] = func(
                            r.test_folder, hash=prev_hash)
                    run_babelrts(babelRTS, r, commit, run_babelrts_tests)
                    if i:
                        commit.changed = get_changed_files(
                            r.commits[i - 1].hash, commit.hash)
...
def run_babelrts(babelRTS, r, commit, run_babelrts_tests):
    if commit.all.tests is None:
        ...
        selected_tests = tuple(
            path for path in selected_tests if path in commit.all.tests)
        if selected_tests:
        ...
        commit.babelrts = so(tests=sorted(selected_tests), duration=duration,
                             dependencies=dependencies, changed=sorted(changed), files=sorted(files))

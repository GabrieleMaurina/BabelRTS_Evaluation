...
def main():
    ...
    java_source_folders = tuple(relpath(source_folder, jdk.path)
                                for source_folder in get_java_source_folders(main_source_folder))
    native_source_folders = tuple(relpath(source_folder, jdk.path)
                                  for source_folder in get_native_source_folders(main_source_folder))

    jdk.java_source_folders = len(java_source_folders)
    jdk.native_source_folders = len(native_source_folders)

    print('RTS')
    babelRTS = BabelRTS(jdk.path, java_source_folders + native_source_folders,
                        TEST_FOLDER, None, LANGUAGES, implementations)
    ...
    for index, sha in enumerate(jdk.shas):
        ...
        if index:
            ...
            commit.java_files = sum(
                1 for file in all_files if file.endswith('.java'))
            commit.native_files = sum(
                1 for file in all_files if not file.endswith('.java'))
